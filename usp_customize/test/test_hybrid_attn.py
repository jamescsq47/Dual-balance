...
if __name__ == "__main__":
    ...
    # data=torch.load("query_key_value_dict.pt", map_location='cpu', weights_only=True)
    ...
    block = 15
    # sparse_data = torch.load("sparse_plan_expanded.pth", map_location='cpu', weights_only=True)
    # sparse = sparse_data['sparse'][0, block, :, :, :].to(device) 

    # sparse_data = torch.load("sparse_expanded.pth", map_location='cpu', weights_only=True)
    ...
    # perm_idx_all = torch.load("perm_idx_all.pt")
    # deperm_idx_all = torch.load("deperm_idx_all.pt")
