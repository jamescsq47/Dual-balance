...
def hybrid_permute_v3(
    ...
    if sparse.dim() == 3:
        ...
        if ulysses_degree == 1:
            ...
            # ensure head_perm_idx is on same device as sparse
            sparse_reordered = sparse.index_select(0, head_perm_idx.to(sparse.device, dtype=torch.long))
...
# hybrid
# sparse_data = torch.load("sparse_expanded.pth", map_location='cpu', weights_only=True) # 0.4141
# sparse = sparse_data['sparse'][0].cuda()  # [40, 40, 1182, 1182]

sparse = torch.load("wan_sparse_0.61.pth", map_location='cpu', weights_only=True).cuda() # 0.61 # [40, 40, 1182, 1182]
...
# random sparse for test
# for block in range(sparse.shape[0]):
#     for head in range(sparse.shape[1]):
#         sparse[block][head] = torch.rand(sparse[block][head].shape, device='cuda') < block/sparse.shape[0]*head/sparse.shape[1]*0.8 + 0.2
# print(sparse.float().sum()/sparse.numel())

# sparse_ratio = 0.414
# sparse = (torch.rand(sparse.shape, device='cuda')< sparse_ratio)
