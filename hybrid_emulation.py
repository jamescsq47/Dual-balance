...
for block in range(40):
    ...
    sparse_final3, head_perm_idx, new_row_perm_idx, new_col_perm_idx, transpose_matrix_q, transpose_matrix_k, head_deperm_idx, new_row_deperm_idx, new_col_deperm_idx = hybrid_permute_v4(sparse_piece, ulysses_degree=ulysses_degree, ring_degree=ring_degree, reward=20)
    imbalance_ratio_list.append(hybrid_imbalance_ratio(sparse_piece, ulysses_degree=ulysses_degree, ring_degree=ring_degree))
    # print(transpose_matrix_q)
    # print(sparse_piece.sum() / (sparse_piece.numel()), sparse_final3.sum() / (sparse_final3.numel()))

    # sparse_final2, head_perm_idx, row_perm_idx, col_perm_idx, head_deperm_idx, row_deperm_idx, col_deperm_idx = hybrid_permute_v2(sparse_piece, ulysses_degree=ulysses_degree, ring_degree=ring_degree)
    new_imbalance_ratio_list.append(hybrid_imbalance_ratio(sparse_final3, ulysses_degree=ulysses_degree, ring_degree=ring_degree))

    print(f"{block}: {hybrid_imbalance_ratio(sparse_piece, ulysses_degree=ulysses_degree, ring_degree=ring_degree)}, {hybrid_imbalance_ratio(sparse_final3, ulysses_degree=ulysses_degree, ring_degree=ring_degree)}")
    print(transpose_matrix_q)

print(f"average: {sum(imbalance_ratio_list) / len(imbalance_ratio_list)}")
print(f"average: {sum(new_imbalance_ratio_list) / len(new_imbalance_ratio_list)}")
